...
<div class="ibox" style="border-radius:5px;padding:7px; height:1000px;">
	<div class="ibox-head">
		...
		<a href="answer-key-report.php" class="btn btn-outline-danger btn-rounded waves-effect" style="font-style:italic;"><< Student Answer Sheet</a>
	</div>	
	...
	<tr>
	<th>Sr.No</th>
	<th><input type="checkbox" name="checkall" id="checkall" style="height:20px; width:20px;" />ALL
	</th> 
	<th>Assign Status</th>
	...
	$data	=	array();
	$data	=	$db->get_all_answer_keys_details($edit_id,$from_date,$to_date,$question_paper);
	...
	if(!empty($data))
	{
		...
		foreach($data as $record)
		{
			...
			<tr> 
				...
				<td><input type="checkbox" name="count_<?php echo $counter; ?>"  id="checkall" style="height:20px; width:20px; cursor:pointer;" class="chk_bx" />
					<input type="hidden" value="<?php echo $res_user_id; ?>" name="res_user_id_<?php echo $counter; ?>">
					<input type="hidden" value="<?php echo $res_question_id; ?>" name="res_question_id_<?php echo $counter; ?>">
				...
				{
				?>
				<a href="images/assign.png" target="_blank" ><img src="images/assign.png" height="15px" width="15px"></a>
				<br />
				Assign To:<span style="color:orange;"><?php echo $staff_name; ?></span>
				<?php
				}
				else
				{
				?>
				<a href="images/notassign.png" target="_blank" ><img src="images/notassign.png" height="10px" width="10px"></a>

				<?php
				}
				?>
				<br />
				Checked Status:<span style="color:blue;"><?php echo $checked_status; ?></span>
				</td>
	...
	<?php
		...
		<select class="form-control" style="width:100%;" name="select_staff">
		<option value="Select">Select  Staff </option>
		<?php
		$report_details = $db->get_all_staff_reg_details_all();
		if(!empty($report_details))
		{
			...
			foreach($report_details as $record)
			{
				...
				$res_name		=	$report_details[$counter][1];
				$email			=	$report_details[$counter][2];
				$contact		=	$report_details[$counter][3];
				$password		=	$report_details[$counter][4];
				$address		=	$report_details[$counter][5];
				
		?>
		<option value="<?php echo $contact; ?>" <?php if($select_staff==$contact){ ?> Selected <?php } ?>><?php echo $res_name; ?></option>
		<?php
				$counter++;
			}
		...
		<div class="col-sm-4 col-md-4 col-lg-4 form-group mb-4">
		<input type="submit" class="btn btn-pink"  style="margin:auto;width:100%;"  
		name="staff_assign" value="ASSIGN ANSWER KEY TO STAFF" onclick="return confirm('Are You Sure for Asign Staff?');" >
		</div>
