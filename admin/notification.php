...
<?php 
	...
	{	
		...
		if($flag==0)
		{
			$db->add_new_notification($title,$actual_image_name);
			$succ_flag = 1 ;
		}
	}
	if(isset($_GET['delete_id']) AND (isset($_GET['image'])))
	{
		 ...
		 $name 	 	= $_GET['image'];
		 $db->delete_notification_images($del_id);
...
<div class="ibox" style="border-radius:5px; padding:7px; margin:auto;">
	...
	<form class="form-pink" method="post" action="<?php echo $_SERVER['PHP_SELF']?>" name="myForm" onsubmit="return validateForm()" autocomplete="off" enctype="multipart/form-data">
		...
		<div class="ibox-head">
			<div class="ibox-title"><i class="fas fa-bell" style="margin-right:10px;"></i>Add Notification</div>
		</div>
		
		<div class="ibox-body">
			<div class="row">
			<div class="col-sm-6 form-group mb-6">
			<label class="form-group mb-4 set-row"><b>Title</b></label>
			<div class="input-group-icon input-group-icon-left  set-row">
			 ...
			 <div class="col-sm-6 form-group mb-6">
			<label class="form-group mb-4 set-row"><b>Attachment</b></label>
			<div class="input-group-icon input-group-icon-left  set-row">
				<span class="input-icon input-icon-left"><i class="fas fa-list"></i></span>
			...
			<div class="col-sm-12 form-group mb-12">
				...
				<center><button class="btn btn-pink btn-air mr-2" type="submit" name="add_btn">Add Notification</button></center>
			</div>
		</div>
	</form>
	</div>
   ...
   <?php 
	...
	<div class="ibox">
		<div class="ibox-body">
			<h5 class="font-strong mb-4"><i class="fas fa-bell" style="margin-right:10px;"></i>Notification Report</h5>
			...
			<table class="table table-bordered table-hover" id="example" style="overflow-x:auto;overflow-y:auto;">
				...
				<tr> 
					...
					?>
					<td><a href="notification.php?delete_id=<?php echo $id;?>&image=<?php echo $image;?>" onclick="return confirm('Are you sure?');"><i class="fas fa-trash"style="color:red;margin-left:20px;"></a></td>
