<?php 
	...
	{	
		...
		if($flag==0)
		{
			$check_category = $db->check_category_exist_or_not_for_update($up_id,$title);
			if($check_category == "")
			{
				$db->update_downloads_category($up_id,$title);
				$success_msg = 1 ;
			}else
			{
				$success_msg=2;
	...
	$report_details = $db->get_all_download_category_by_id($up_id);
...
<script type="text/javascript">
	alert("Download Category Updated Successfully");
...
?>
<script type="text/javascript">
	alert("Failed To Update Download Category");
</script>
<?php 
} 
?>
<div class="ibox" style="border-radius:5px; padding:7px; margin:auto;">
	...
	<form class="form-pink" method="post" action="<?php echo $_SERVER['PHP_SELF']?>" name="myForm" onsubmit="return validateForm()" autocomplete="off" enctype="multipart/form-data">
		...
		<div class="ibox-head">
			<div class="ibox-title"><i class="fas fa-cloud-download-alt" style="margin-right:10px;"></i>Update Download Category</div>
			<a href="downloads-category.php" class="btn btn-outline-danger btn-rounded waves-effect" style="font-style:italic;"><< Download Category Report </a>
		</div>
		...
		<div class="ibox-body">
			...
			<div class="col-sm-6 form-group mb-6">
				...
				<center><button class="btn btn-pink btn-air mr-2" type="submit" name="add_btn">Update Download Category</button></center>
			</div>
		</div>
	</form>
	</div>
